...
public final class QueryUtils {
    ...
    /**
     * Return a list of {@linkEarthquake} objects that has been built up from
    ...
    public static ArrayList<QuakeData> extractEarthquakes() {
        ...
        try {
            ...
            //Get the instance JSONObject
            JSONObject jsonRootObject = new JSONObject(SAMPLE_JSON_RESPONSE);

            // Get the instance of JSONArray which contains JSONObjects
            JSONArray earthquakeArray = jsonRootObject.optJSONArray("features");

            //Iterate JSON array and opt the data required
            for (int i = 0; i < earthquakeArray.length(); i++) {
                ...
                QuakeData quakeData = new QuakeData(magnitude, place, time, url);
                earthquakes.add(quakeData);
            }
